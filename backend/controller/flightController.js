...
export const filterflightController = async(req,res)=>{
  try{
         const categoryList = req?.body?.category || []
 
         const product = await Flight.find({
             category :  {
                 "$in" : categoryList
             }
         })
 ...
 export const getAllFlights = async (req, res) => {
   try {
     // Fetch all flights without any filters
     const flights = await Flight.find();
 
     if (flights.length === 0) {
       return res.status(404).json({ status: false, message: "No flights available" });
     }
     ...
     const flightsWithAirlineInfo = await Promise.all(
       flights.map(async (flight) => {
         ...
         // Extract necessary airline information
         return {
           ...populatedFlight.toObject(), // Convert Mongoose document to plain JavaScript object
           airlineLogo: populatedFlight.airline.airlineLogo,
         };
